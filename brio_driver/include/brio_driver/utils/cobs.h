...
class COBS {
 public:
  ...
  static std::vector<uint8_t> Encode(std::vector<uint8_t> buffer) {
    auto encodedBuffer =
        std::vector<uint8_t>(GetEncodedBufferSize(buffer.size()));
  ...
  static std::vector<uint8_t> Decode(std::vector<uint8_t> buffer) {
    auto decodedBuffer = std::vector<uint8_t>(buffer.size());
